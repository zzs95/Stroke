...
from .slowfast_temporal_module import SlowFastSpatialTemporalModule
...
class ResNet_I3D_SlowFast(nn.Module):
    ...
    def __init__(self,
        ...
        self.temporal_module = SlowFastSpatialTemporalModule()
    ...
    def forward(self, x):
        ...
        if len(outs) == 1:
            if self.fast_only:
                return F.adaptive_avg_pool3d(outs[0], 1).view(x.size(0), -1)
            return self.temporal_module(outs[0]).view(x.size(0), -1)
        else:
            return tuple(outs)
