...
metadata_df = metadata_df.merge(series_desc, on='SeriesDescription')

sampled_df = pd.concat([df.sample(n=1950, replace=False, random_state=0) for k,df in metadata_df.groupby('label')])

failures = []
for seri, acch in tqdm(zip(sampled_df['series'], sampled_df['AccessionNumber']), total=len(sampled_df)):
    try:
